...
DEFAULT_SYSTEM_PROMPT = '以下の基本情報も参照して回答してください。\n基本情報：'
...
class ELYZA_srv:
    def __init__(self):
        #https://.co/elyza/Llama-3-ELYZA-JP-8B
...
#        self.model_name = 'weblab-GENIAC/Tanuki-8B-dpo-v1.0'
    ...
    def response (self, prompt, text):
        prompt += (DEFAULT_SYSTEM_PROMPT + self.base_info + '\n')
        output = self.base_response (prompt, text) 
        print(output)
        return output
...
def updateUtter():
    ...
    #response
    res = llm_motel.response(prompt, text)
