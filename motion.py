...
def set_level_motion(level):
    ...
    keyinp.send(motion_list[level])
#    print("keyinp.send(motion_list[level])")

#GOOD JOBモーション呼び出し
def set_goodjob_motion():
    keyinp.send(motion_list[16])
#    print("keyinp.send(motion_list[16])")

#バイバイモーション呼び出し
def set_byebye_motion():
    keyinp.send(motion_list[2])
#    print("keyinp.send(motion_list[2])")
...
if __name__ == '__main__':
    ...
    if 1 <= len(args):
        ...
        if os.path.isfile(sound_list[int(args[1])]) == True:
            ...
            #モーションズレ補正
            time.sleep(0.3)
            keyinp.send(dance_list[int(args[1])])
